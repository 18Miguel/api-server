...
export class SQLComparisonBuilder<Entity> {
    private readonly comparisonMap: Record<string, string>;
    public static DEFAULT_COMPARISON_MAP: Record<string, string> = {
        eq: '=',
        neq: '!=',
        gt: '>',
        gte: '>=',
    ...
    private randomString(): string {
        const uuidBytes = randomBytes(16);
        uuidBytes[6] = (uuidBytes[6] & 0x0F) | 0x40;
        uuidBytes[8] = (uuidBytes[8] & 0x3F) | 0x80;
        const uuid = uuidBytes.toString('hex');
        return uuid;
    }
    
    get paramName() {
        return `param${this.randomString()}`;
    }
     ...
     */
    public build<F extends keyof Entity>(field: F, cmp: FilterComparisonOperators<Entity[F]>, val: EntityComparisonField<Entity, F>, alias?: string): CmpSQLType {
        const col = alias ? `${alias}.${field.toString()}` : `${field.toString()}`;
        const normalizedCmp = cmp.toString().toLowerCase();
    ...
    private isNotComparisonSQL(col, val) {
        if (val === null) {
    ...
    private inComparisonSQL(col, val) {
    ...
    private notInComparisonSQL(col, val) {
        this.checkNonEmptyArray(val);
        const { paramName } = this;
        return {
    ...
    private checkNonEmptyArray(val) {
    ...
    private betweenComparisonSQL(col, val) {
    ...
    private notBetweenComparisonSQL(col, val) {
        if (this.isBetweenVal(val)) {
            const { paramName: lowerParamName } = this;
            const { paramName: upperParamName } = this;
            return {
